...
robinsonUnify(TAbs(t1), TAbs(t2))
@test robinsonUnify(TAbs(t1), TAbs(t2)) isa Error
...
# eq_constraints(simplify(t1, t2), [DirectConstraint(TLoc(3), TAbs(TApp([TProd([TGlob("G3")]), TLoc(1)])))])
...
t1 = TApp([TProd([TGlob("X"), TGlob("Y")]), TAbs(TAppAuto(TGlob("F"), TLoc(1)))])
t2 = TApp([TProd([TGlob("Y"), TGlob("X")]), TAbs(TAppAuto(TGlob("F"), TLoc(2)))])
...
t1 = TApp([TProd([TGlob("X"), TGlob("Y")]), TAbs(TAppAuto(TGlob("F"), TLoc(1)))])
t2 = TApp([TProd([TGlob("X"), TGlob("Y")]), TAbs(TAppAuto(TGlob("F"), TLoc(2)))])
# @test simplify(t1, t2) == Error("Different: X is really different from Y")
robinsonUnify(TAbs(t1), TAbs(t2))
@test robinsonUnify(TAbs(t1), TAbs(t2)) isa Error

t1 = TApp([TProd([TLoc(3), TLoc(2)]), TAbs(TAppAuto(TGlob("F"), TLoc(1)))])
t2 = TApp([TProd([TLoc(1), TLoc(4)]), TAbs(TAppAuto(TGlob("F"), TLoc(2)))])
# @test simplify(t1, t2) == [DirectConstraint(TLoc(3), TLoc(4))]
robinsonUnify(TAbs(t1), TAbs(t2))
@test test_unify_imply(t1, t2)
@test test_unify_join(t1, t2)

t1 = TApp([TProd([TGlob("X"), TLoc(2)]), TAbs(TAppAuto(TLoc(2), TLoc(1)))])
t2 = TApp([TProd([TLoc(1), TLoc(4)]), TAbs(TAppAuto(TGlob("F"), TLoc(2)))])
...
t1 = TAbs(TTermAuto(TLoc(1), TProd([TGlob("A"), TLoc(2)])))
...
e = TAbs(TApp([TProd([TLoc(1), TLoc(1)]), TLoc(2)]))
...
ea = TProd([TAnno(TLoc(1), TGlob("A"))])
ef1 = TGlob("f", TTermAuto(TLoc(1), TProd([TGlob("B1"), TGlob("B2")])))
...
vec_w_proj2_1 = TProd([TApp([TLoc(1), TAbs(TLoc(2))]), TLoc(2)])
proj1_1 |> pr
vec_w_proj2_1 |> pr
infer_type_rec(proj1_1) |> pr_ctx
infer_type_rec(vec_w_proj2_1) |> pr_ctx
e = TProd([proj1_1, vec_w_proj2_1])
e |> pr
