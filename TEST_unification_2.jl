...
# include("TEST_unification_2.jl") # itself
...
ass_reduc(t1, s1) |> pr
ass_reduc(t2, s2) |> pr
@test ass_reduc(t1, s1) == TProd(Type_[TGlob("A"), TGlob("B"), TLoc(1)])
...
e = ELoc(2)
@test infer_type_rec(e) == Inf_res(Type_[TLoc(1), TLoc(2)], TLoc(2))

e = EGlob("f", TTermAuto(TGlob("A"), TGlob("B")))
@test infer_type_rec(e) == Inf_res(Type_[], TTermAuto(TGlob("A"), TGlob("B")))

e = EAnno(ELoc(1), TGlob("A"))
@test infer_type_rec(e) == Inf_res(Type_[TGlob("A")], TGlob("A"))

e = EAnno(ELoc(2), TGlob("A"))
@test infer_type_rec(e) == Inf_res(Type_[TLoc(1), TGlob("A")], TGlob("A"))
...
@test infer_type_rec(e) == Inf_res(Type_[], TProd(Type_[TTermAuto(TGlob("A"), TGlob("B")), TTermAuto(TGlob("A"), TGlob("B"))]))

tt = TForall(TTermAuto(TLoc(1), TGlob("B")))
e = EProd([EGlob("f", tt), EGlob("g", tt)])
@test infer_type_rec(e) == Inf_res(Type_[], TProd(Type_[TTerm(TProd(Type_[TLoc(1)]), TGlob("B")), TTerm(TProd(Type_[TLoc(2)]), TGlob("B"))]))
infer_type_rec(e).res_type |> pr # == "[T1->B x T2->B]" # T2, important! GOOD

e = EProd([ELoc(2), ELoc(2)])
@test infer_type_rec(e) == Inf_res(Type_[TLoc(1), TLoc(2)], TProd(Type_[TLoc(2), TLoc(2)]))

e = EProd([ELoc(2), EAnno(ELoc(2), TLoc(4))])
@test infer_type_rec(e) == Inf_res(Type_[TLoc(1), TLoc(5)], TProd(Type_[TLoc(5), TLoc(5)]))


e = EProd([EAnno(ELoc(1), TLoc(3)), EAnno(ELoc(1), TLoc(4))])
@test infer_type_rec(e) == Inf_res(Type_[TLoc(6)], TProd(Type_[TLoc(6), TLoc(6)]))


e = EProd([EAnno(ELoc(1), TLoc(3)), EAnno(ELoc(1), TLoc(4)), EAnno(ELoc(2), TLoc(5))])
@test infer_type_rec(e) == Inf_res(Type_[TLoc(6), TLoc(11)], TProd(Type_[TLoc(6), TLoc(6), TLoc(11)]))

e = EAbs(EProd([ELoc(2), EAnno(ELoc(1), TGlob("T"))]))
@test infer_type_rec(e) == Inf_res(Type_[], TTerm(TProd(Type_[TGlob("T"), TLoc(1)]), TProd(Type_[TLoc(1), TGlob("T")])))

e = EProd([EAnno(ELoc(1), TLoc(2)), EAnno(ELoc(1), TLoc(3)), EAnno(ELoc(1), TLoc(4))])
@test infer_type_rec(e) == Inf_res(Type_[TLoc(7)], TProd(Type_[TLoc(7), TLoc(7), TLoc(7)]))

e = EAnno(EAbs(EGlob("b", TGlob("B"))), TTermAuto(TProd([TGlob("A")]),  TGlob("B")))
infer_type_rec(e).res_type |> pr
@test infer_type_rec(e) == Inf_res(Type_[], TTerm(TProd(Type_[TProd(Type_[TGlob("A")])]), TGlob("B")))
...
@test infer_type_rec(e) == Inf_res(Type_[TGlob("A")], TGlob("B"))

e = EAbs(EApp([EProd([ELoc(1), ELoc(1)]), ELoc(2)]))
infer_type_rec(e).res_type |> pr # == "[T1 x [T1 x T1]->T2]->T2"
@test infer_type_rec(e).res_type == TTerm(TProd([TLoc(1), TTerm(TProd([TLoc(1), TLoc(1)]), TLoc(2))]), TLoc(2))
...
e = EAbs(EApp([ea, ef1]))
e |> pr
@test infer_type_rec(e) == Inf_res(Type_[], TTerm(TProd(Type_[TGlob("A")]), TGlob("B")))
infer_type_rec(e).res_type |> pr
