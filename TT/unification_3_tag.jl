...
err_msg_prods_tags(t1::TProdTag, t2::TProdTag) = "Some tags are in second, not in first: $(keys(t1.data_tags)) < $(keys(t2.data_tags)), so you cannot even drop them."
...
err_msg_tags(t1::TProdTag, t2::TProdTag) = "You cant unify var_tags: $(t1) != $(t2)."
...
function meetjoin_rec_unify_(t1::TAppTag, t2::TAppTag, do_meet::Bool)::MeetJoin_rec_res
    if length(t1.ops_dot_ordered) != length(t2.ops_dot_ordered)
        return MeetJoin_rec_res(TermTagwError(t1, Error(err_msg_app(t1, t2))), Array{Constraint}[])
    end
...
concat_(t::TProdTag...) = TProdTag(vcat((t .|> (x -> x.data))...), merge((t .|> (x -> x.data_tags))...))
subdict(d::Dict{Id,TermTag}, keys::Set{Id}) = Dict{Id,TermTag}(k => d[k] for k in keys)
...
function meetjoin_rec_unify_(t1::TProdTag, t2::TProdTag, do_meet::Bool)::MeetJoin_rec_res
    ...
    res_data_types = Array{TermTag}(all_ress .|> x -> x.res_type)
    ...
    MeetJoin_rec_res(TProdTag(res_data_types, res_types_tags), vcat(res_tags_cs, res_data_cs))
end
...
function meetjoin_rec_unify_(t1::TSumTag, t2::TSumTag, do_meet)::MeetJoin_rec_res
    ...
    if length(errors) > 0
        res_type = TermTagwError(res_type, "E" * join(errors .|> string, ""))
    end
...
function meetjoin_rec_unify_(t1::TAbsTag, t2::TAbsTag, do_meet)::MeetJoin_rec_res
    ...
    if t1 == t2
        ...
        MeetJoin_rec_res(TermTagwError(t1, Error(err_msg_lambdas(t1, t2))), Array{Constraint}[])
    end
end
...
function meetjoin_rec_unify_(t1::TSumTermTag, t2::TSumTermTag, do_meet)::MeetJoin_rec_res
    if t1.tag != t2.tag
        return MeetJoin_rec_res(TermTagwError(t1, Error(err_msg_sumtags(t1, t2))), Array{Constraint}[])
...
function meetjoin_rec_unify_(t1::TLocStrTag, t2::TLocStrTag, do_meet)::MeetJoin_rec_res
...
function meetjoin_rec_unify_(t1::TermTag, t2::TermTag, do_meet)::MeetJoin_rec_res # base case
    if t1 == t2
        MeetJoin_rec_res(t1, Array{Constraint}([]))
    elseif t1 isa TLocTag
        MeetJoin_rec_res(t1, Array{Constraint}([EqConstraint(t1, t2)]))
    elseif t2 isa TLocTag
        MeetJoin_rec_res(t2, Array{Constraint}([EqConstraint(t2, t1)]))
    else
        MeetJoin_rec_res(TermTagwError(t1, Error(err_msg_terms(t1, t2))), Array{Constraint}[])
    end
end
...
function imply_unify_(t1::TermTag, t2::TermTag)::Imply_res # base case
    if t1 == t2
        Array{Constraint}([])
    elseif typeof(t1) === TLocTag || typeof(t2) === TLocTag
        Array{Constraint}([DirectConstraint(t1, t2)])
    else
...
id_data(current_arity) = Array{TermTag}([TLocTag(i) for i in 1:current_arity])
id_tags(current_tags) = Dict{Id,TermTag}(i => TLocStrTag(i) for i in current_tags)

struct Arity
    data::Int
    tags::Set{String}
end
get_arity_obj(t::TermTag) = Arity(t |> arity, t |> usedLocsSet)


function get_subst_prod_data(tloc::TLocTag, tt::TermTag, curr_arity::Arity)::TProdTag
    # resulting_arity = curr_arity.data - 1
    # you have ALREADY TESTED that tt does not contain tloc, that's the whole point !!!!
    prod = vcat(
        Array{TermTag}([TLocTag(i) for i = 1:(tloc.var-1)]),
        Array{TermTag}([TLocTag(0)]), # Placeholder, complete nonsense, it's getting replaced
        Array{TermTag}([TLocTag(i) for i in (tloc.var:curr_arity.data-1)])
    )
    prod[tloc.var] = ass_reduc(tt, TProdTag(prod))
    TProdTag(prod, id_tags(curr_arity.tags))
end  # THIS IS DIFFERENT IN VAR VS VAR_TAG #
function get_subst_prod_tag(tloc::TLocStrTag, tt::TermTag, curr_arity::Arity)::TProdTag
    # you have ALREADY TESTED that tt does not contain tloc, that's the whole point !!!!
    subst = id_tags(curr_arity.tags)
    subst[tloc.var_tag] = tt # i'm PRETENDING THAT tt DOES NOT CONTAIN var_tag
    TProdTag(id_data(curr_arity.data), subst)
end  # THIS IS DIFFERENT IN VAR VS VAR_TAG #


function share_ctx_tlocs_names(t1::TAbsTag, t2::TAbsTag, t1arity::Index, t2arity::Index)
    s1 = TProdTag(Array{TermTag}([TLocTag(i) for i = 1:t1arity]))
    s2 = TProdTag(Array{TermTag}([TLocTag(i) for i = (t1arity+1):(t1arity+t2arity)]))
...
function get_loc_loc_constraint(v)
    for i = 1:length(v)
        if !(v[i] isa ErrorConstraint) && (v[i].t1 isa TLocTag && v[i].t2 isa TLocTag)
            return i
        end
    end
    nothing
...
function robinsonUnify(t1::TAbsTag, t2::TAbsTag, t1arity::Arity, t2arity::Arity; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_)::Union{Succeded_unif_res,Failed_unif_res,ItsLiterallyAlreadyOk}
    ...
    while (i = get_loc_loc_constraint(STACK)) !== nothing
        ...
        current_arity, current_total_subst = do_the_subst_thing!(TLocTag(max(l1, l2)), TLocTag(min(l1, l2)), current_arity, current_total_subst, STACK, ERRORSTACK; data = true)
    end
    ...
    while (length(STACK) > 0)
        c = pop!(STACK)
        ct1, ct2 = c.t1, c.t2
        if !(ct1 isa TLocTag || ct2 isa TLocTag) && !(ct1 isa TLocStrTag || ct2 isa TLocStrTag)
            @assert mode == imply_
            ...
            continue
        elseif ct1 isa TLocStrTag && ct2 isa TLocStrTag
        ...
        elseif ct1 isa TLocStrTag
            @assert !(ct2 isa TLocTag)
        ...
        elseif ct2 isa TLocStrTag
            @assert !(ct1 isa TLocTag)
    ...
    subst1 = TProdTag(final_subst.data[1:t1arity.data], final_subst.data_tags)
    subst2 = if unify_tlocs_ctx
        TProdTag(final_subst.data[(t1arity.data+1):(t1arity.data+t2arity.data)], final_subst.data_tags)
    else
        TProdTag(final_subst.data[1:t2arity.data], final_subst.data_tags)
    end
    # subst1, subst2 = final_subst, final_subst
    res_type = if (mode == imply_)
        TGlobTag("O") # Why nothing: USE >>t2<< ( The Original one, NOT the Shared version)
    else
...
robinsonUnify(t1::TAbsTag, t2::TermTag, t1arity::Arity, t2arity::Arity; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(t1, TAbsTag(t2), t1arity, t2arity; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
robinsonUnify(t1::TermTag, t2::TAbsTag, t1arity::Arity, t2arity::Arity; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(TAbsTag(t1), t2, t1arity, t2arity; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
function robinsonUnify(t1::TermTag, t2::TermTag, t1arity::Arity, t2arity::Arity; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_)
    if (t1arity == 0) && (t2arity == 0)
        return true #(t1 == t2) ? (TProdTag([]), TProdTag([])) : Error(" Not unifiable: $(t1) != $(t2)")
    else
        return robinsonUnify(TAbsTag(t1), TAbsTag(t2), t1arity::Arity, t2arity::Arity; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
    end
end


# All cases WITHOUT precomputed tarities:
robinsonUnify(t1::TAbsTag, t2::TAbsTag; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(t1, t2, t1.body |> get_arity_obj, t2.body |> get_arity_obj; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
robinsonUnify(t1::TAbsTag, t2::TermTag; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(t1, TAbsTag(t2), t1.body |> get_arity_obj, t2 |> get_arity_obj; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
robinsonUnify(t1::TermTag, t2::TAbsTag; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(TAbsTag(t1), t2, t1 |> get_arity_obj, t2.body |> get_arity_obj; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
robinsonUnify(t1::TermTag, t2::TermTag; unify_tlocs_ctx::Bool = true, mode::Unify_mode = join_) = robinsonUnify(TAbsTag(t1), TAbsTag(t2), t1 |> get_arity_obj, t2 |> get_arity_obj; unify_tlocs_ctx = unify_tlocs_ctx, mode = mode)
...
function infer_type_(term::TLocTag)::TermTag
    TTermTag(TProdTag(Array{TermTag}([TLocTag(i) for i = 1:term.var])), TLocTag(term.var))
end #  (but also kinda this is right)
function infer_type_(term::TLocStrTag)::TermTag
    return TTermTag(TProdTag(Dict{Id,TermTag}(term.var_tag => TLocTag(1))), term)  # TAbsTag(TLocTag(term.var)) was an idea i tried
end #  (but also kinda this is right)
...
function infer_type_(term::TTopTag)::TermTag
    return TTermEmpty(TTopTag())
end
function infer_type_(term::TAnnoTag, t_computed::TTermTag)::TermTag # IMPORTANT: if an error comes up, THIS FUNCTION will turn res into TermTagwError
    ...
    if substs isa ItsLiterallyAlreadyOk
        return TTermTag(t_computed.t_in, term.type)
    end
    term_type = if term.type isa TAbsTag
        term.type.body
    else
        term.type
    end   # Oh fuck what am i doing
    args = if (t_computed.t_in.data |> length == 0)
        TProdTag(Array{TermTag}([]))
    else
    ...
    if substs isa Failed_unif_res
        res = TermTagwError(res, Error("Wrong annotation: " * get_string(subst[4])))
    end
...
function infer_type_(term::TProdTag, ts_computed::Array{TTermTag})::TermTag # IMPORTANT: if an error comes up, THIS FUNCTION will turn res into TermTagwError
    ...
    if length(all_errors) > 0 # Or there's some error into res !!
        res = TermTagwError(res, Error("Impossible to unify args of this prod: " * get_string(all_errors)))
    end
    res
end
...
function infer_type_(term::TSumTermTag, t_computed::TTermTag)::TermTag
    ...
    types = vcat([TLocTag(n) for n = (arT+1):(arT+tag-1)], [t_computed.t_out])
...
function infer_type_(term::TAppTag, ts_computed::Array{TTermTag})::TermTag
    ...
    for t in ts_computed[2:end]
        ...
        if tterm_subst isa Failed_unif_res
            return TermTagwError(TAppTag(ts_computed .|> (x -> x.t_out)), Error("Calling non function: " * get_string(tterm_subst[4]))) #y'know what? Yes this is violent.. I don't care
        elseif tterm_subst isa ItsLiterallyAlreadyOk
            push!(ts_computed_2, t)
        else
            push!(ts_computed_2, ass_reduc(t, tterm_subst[1])) # t.t_out SHOULD be nothing else but a TTermTag... RIGTH?
        end
    end
    ...
    for i = 2:length(tterms)
        ...
        # : i DONT LIKE these TAbsTag, it's WRONG, but, it's the only way of accessing unify_tlocs_ctx atm
        # NOTE^ :  mode=imply_ doesnt even return a res_type, even less one with an error! Of course
        if substs isa ItsLiterallyAlreadyOk
    ...
    if length(all_errors) > 0
        res = TermTagwError(res, Error("Type of the application don't match: " * get_string(all_errors)))
    end
...
function infer_type_rec(term::TTopTag)::TermTag
    return infer_type_(term)
end
...
function infer_type_rec(term::TAbsTag)::TermTag
...
function infer_type_rec(term::TSumTermTag)::TermTag
